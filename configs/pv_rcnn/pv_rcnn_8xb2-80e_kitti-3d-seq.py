...
val_evaluator = dict(
    type='KittiMetric',  # KITTI数据集专用评估器
    ann_file=data_root + 'kitti_infos_val.pkl',  # 验证集标注文件路径
    metric='bbox',  # 评估3D边界框指标
    pcd_limit_range=point_cloud_range,  # 关键：与点云范围一致，过滤超出范围的预测
    backend_args=backend_args  # 与数据加载的后端参数一致
)
# 测试集评估器通常与验证集保持一致
