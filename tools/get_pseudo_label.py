...
# 子目录路径
BIN_DIR = '/home/users/weiyan/go2nus_kitti/pointcloud'
...
SAVE_DIR = {
    'kitti_annot': '/home/users/weiyan/go2nus_kitti/pseudo_label',  # 保存KITTI格式标注文件
    'json_result': './results',            # 保存JSON统计结果
    'npy_data': './npy_data'               # 保存NPY数值数据
}
def save_kitti_annot(file_id, annot_lines):
    """保存KITTI格式标注到文件"""
    save_path = os.path.join(SAVE_DIR['kitti_annot'], f"{file_id}.txt")
    with open(save_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(annot_lines))
    print(f"【{file_id}】KITTI标注已保存至：{save_path}")
...
def process_single_bin(idx):
    ...
    try:
        ...
        save_kitti_annot(file_id, final_kitti_annot_lines)
...
if __name__ == "__main__":
    ...
    for idx in tqdm(range(MAX_FILE_NUM)):
